...
{
    [CustomEditor(typeof(IsometricRuleTileOverride), true)]
    ...
    internal class IsometricRuleTileEditor : RuleTileEditor
    {
        ...
        {
            var isoTile = (IsometricRuleTileOverride) ruleTile;
            ...
            for (int y = 0; y <= 2; y++)
            {
                for (int x = 0; x <= 2; x++)
                {
                    ...
                    if (x != 1 || y != 1)
                    {
                        RuleOnGUI(r, new Vector3Int(x,y), tilingRule.m_Neighbors[index]);
